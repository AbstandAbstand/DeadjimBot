...
function acceptNewPayment(payment){
  ...
  let amount = payment["additional_data"]["amount"]["amount"];
  let currency = payment["additional_data"]["amount"]["currency"];
  let hash = payment["additional_data"]["hash"];
  
  let USDAmount = convertToUSD(amount, currency);
  let res = Libs.ResourcesLib.userRes("deposit");
  let res.add(USDAmount);
